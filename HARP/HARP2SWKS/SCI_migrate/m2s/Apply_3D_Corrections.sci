...
if mtlb_logic(round(100*r_over_R),"<=",35) then
  r_over_R = 0.35;
end;
...
mod_TSR = (((Omg*RotorRad)*%pi)/30)/sqrt(mtlb_a(Vavg^2,(((Omg*RotorRad)*%pi)/30)^2));
...
Lift_Slope = mtlb_s(y2,y1)/(x2-x1);//units [1/deg]
...
// !! L.75: Unknown function FindInd not converted, original calling sequence used.
ii = FindInd(Alpha,90);
adj = zeros(ii,1);
DelCL = zeros(ii,1);
DelCD = zeros(ii,1);
//Make corrections only in the range of Alpha = -10 to 90 degrees
// !! L.80: Unknown function FindInd not converted, original calling sequence used.
// !! L.80: Unknown function FindInd not converted, original calling sequence used.

for n = mtlb_imp(FindInd(Alpha,-10),FindInd(Alpha,90))
  ...
  CL_p = Lift_Slope*mtlb_s(mtlb_e(Alpha,n),Alpha_zero);
  ...
  FL = mtlb_s((((1.6*c_over_r)/0.1267)*mtlb_s(a,c_over_r^Expon))/mtlb_a(b,c_over_r^Expon),1)/(Lift_Slope/d2r);

  if mtlb_logic(mtlb_e(Alpha,n),"<",Alpha_End) then
    CL_3D = mtlb_i(CL_3D,n,mtlb_a(mtlb_e(CL_2D,n),FL*mtlb_s(CL_p,mtlb_e(CL_2D,n))));
  else
    //This adjustment factor was defined by C. Hansen (Windward Engineering)
    //for the AirfoilPrep worksheet
    adj = mtlb_i(adj,n,(mtlb_s(90,mtlb_e(Alpha,n))/(90-Alpha_End))^2);
    CL_3D = mtlb_i(CL_3D,n,mtlb_a(mtlb_e(CL_2D,n),(FL*mtlb_s(CL_p,mtlb_e(CL_2D,n)))*adj(n)));
  end;
  //=======================================================================%

  if mtlb_logic(Correct_3D,"==",1) then
    //%Apply 3D corrections to the drag coefficients (Selig-Du method)
    //=======================================================================%
    dExpon = Expon/2;
    FD = mtlb_s((((1.6*c_over_r)/0.1267)*mtlb_s(a,c_over_r^dExpon))/mtlb_a(b,c_over_r^dExpon),1)/(Lift_Slope/d2r);
    CD_3D = mtlb_i(CD_3D,n,mtlb_s(mtlb_e(CD_2D,n),FD*mtlb_s(mtlb_e(CD_2D,n),Drag_zero)));
  
    //Sometimes the stall model goes crazy at a small r/R values and
    //creates negative drags, this forces the drag to be zero if
    //negative
    if mtlb_logic(mtlb_e(CD_3D,n),"<",0) then
      CD_3D = mtlb_i(CD_3D,n,0);
    end;
    //=======================================================================%
  else
    //%Apply 3D corrections to the drag coefficients (Eggars method)
    //=======================================================================%     
    DelCL = mtlb_i(DelCL,n,mtlb_s(mtlb_e(CL_3D,n),mtlb_e(CL_2D,n)));
    DelCD = mtlb_i(DelCD,n,(DelCL(n)*mtlb_s(sin(mtlb_e(Alpha,n)*d2r),0.12*cos(mtlb_e(Alpha,n)*d2r)))/mtlb_a(cos(mtlb_e(Alpha,n)*d2r),0.12*sin(mtlb_e(Alpha,n)*d2r)));
    CD_3D = mtlb_i(CD_3D,n,mtlb_a(mtlb_e(CD_2D,n),DelCD(n)));
    if mtlb_logic(mtlb_e(CD_3D,n),"<",0) then //Make sure the drag coefficient never becomes negative, which sometimes happens when using these stall-delay models
     CD_3D = mtlb_i(CD_3D,n,0);
    end;
    //=======================================================================% 
  end;
...
CL_3D = mtlb_i(CL_3D,isnan(CL_3D),0);
CD_3D = mtlb_i(CD_3D,isnan(CD_3D),0);
