...
for n = 2:NumSeg
    Twist_diff = abs((TWIST(n:$) - TWIST(n-1))); // units: radians
    r_arm = RElm(n:$) - RElm(n-1); // moment arm (m)
    Mnorm(n-1,1) = sum((Fn(n:$).*cosd(Twist_diff)...
                      + Ft(n:$).*sind(Twist_diff)).*r_arm)./1000; // (kN-m)
    Mtan(n-1,1)  = sum((Fn(n:$).*sind(Twist_diff)...
                      - Ft(n:$).*cosd(Twist_diff)).*r_arm)./1000; // (kN-m)
    Mflap(n-1,1) = sum((Lift(n:$).*sind(AFang_n(n:$))...
                      + Drag(n:$).*cosd(AFang_n(n:$))).*r_arm)./1000; // (kN-m)
    Medge(n-1,1) = sum((Lift(n:$).*cosd(AFang_n(n:$))...
                      - Drag(n:$).*sind(AFang_n(n:$))).*r_arm)./1000; // (kN-m)
end
...
//      STmax = CHORD(N)*PERCENT_THICKNESS(N)/200;
    STmax = max(Scaled_AF_Coordinates{N}(1:NumAFPoints/2,2) - ...
                flipud(Scaled_AF_Coordinates{N}(NumAFPoints/2:$,2)))/2;
